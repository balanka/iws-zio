...
import zio._
import zio.http.api.{EndpointExecutor, EndpointRegistry, EndpointSpec, MiddlewareSpec, RouteCodec}
...
object APIExamples extends ZIOAppDefault {
  ...
  val getUsers = EndpointSpec.get("users" / int).out[Int]

  val getUserEndpoint = getUsers.implement { id =>
      ZIO.succeed(id)
    }

  val getUserPosts = EndpointSpec
