...
object BankStmtEndpoint {
  ...
  val bsModifyAPI     = Endpoint.post(literal("bs")/ string("company")).in[BankStatement].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val bsModifyEndpoint = bsModifyAPI.implement(p => ZIO.logInfo(s"Modify BankStatement  ${p}") *>
    BankStatementRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
