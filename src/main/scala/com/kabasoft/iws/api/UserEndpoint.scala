...
object UserEndpoint {
 ...
 val userModifyAPI     = Endpoint.post(literal("user")/ string("company")).in[User].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val userModifyEndpoint = userModifyAPI.implement(p => ZIO.logInfo(s"Modify user  ${p}") *>
    UserRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
