...
object VatEndpoint {
  ...
  val vatModifyAPI     = Endpoint.post(literal("vat")/ string("company")).in[Vat].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val vatModifyEndpoint = vatModifyAPI.implement(p => ZIO.logInfo(s"Modify vat  ${p}") *>
    VatRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
