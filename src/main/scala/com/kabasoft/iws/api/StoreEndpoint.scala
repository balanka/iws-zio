...
object StoreEndpoint {
  ...
  val storeByIdAPI       = Endpoint.get("store" / string("id")/ string("company")).out[Store].outError[RepositoryError](Status.InternalServerError)
  ...
  val appStore = storeAllEndpoint ++ storeByIdEndpoint  ++ storeCreateEndpoint ++storeDeleteEndpoint++ storeModifyEndpoint
