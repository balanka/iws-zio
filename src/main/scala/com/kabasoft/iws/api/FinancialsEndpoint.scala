...
object FinancialsEndpoint {
  ...
  private val ftrByTransIdAPI  = Endpoint.get("ftr" / string("company")/ int("transid")).out[FinancialsTransaction].outError[RepositoryError](Status.InternalServerError)
  ...
  private val ftrPost4PeriodAPI     = Endpoint.get("ftr/post"/ string("company")/ int("from") / int("to")).out[List[Int]].outError[RepositoryError](Status.InternalServerError)
  ...
  private val ftrByTransIdEndpoint = ftrByTransIdAPI.implement( p => FinancialsTransactionCache.getByTransId((p._2.toLong, p._1)).mapError(e => RepositoryError(e.getMessage)))
