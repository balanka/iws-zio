...
object SupplierEndpoint {
  ...
  val supModifyAPI     = Endpoint.post(literal("cust")/ string("company")).in[Supplier].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val supModifyEndpoint = supModifyAPI.implement(p => ZIO.logInfo(s"Modify supplier  ${p}") *>
    SupplierRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
