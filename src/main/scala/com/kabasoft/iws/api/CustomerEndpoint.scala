...
import com.kabasoft.iws.repository.Schema.{customerSchema, repositoryErrorSchema}
...
object CustomerEndpoint {
  ...
  val custAllAPI         = Endpoint.get("cust"/  int("modelid")/string("company")).out[List[Customer]].outError[RepositoryError](Status.InternalServerError)
