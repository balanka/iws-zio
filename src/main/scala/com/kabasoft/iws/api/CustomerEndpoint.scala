...
object CustomerEndpoint {
  ...
  val custModifyAPI     = Endpoint.post(literal("cust")/ string("company")).in[Customer].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val ccModifyEndpoint = custModifyAPI.implement(p => ZIO.logInfo(s"Modify customer  ${p}") *>
    CustomerRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
