...
import com.kabasoft.iws.repository.Schema.{moduleSchema, repositoryErrorSchema}
...
object ModuleEndpoint {
  ...
  val moduleAllAPI         = Endpoint.get("module"/int("modelid")/ string("company")).out[List[Module]].outError[RepositoryError](Status.InternalServerError)
  ...
  val moduleAllEndpoint      = moduleAllAPI.implement(p => ZIO.logInfo(s"All module  ${p}") *>ModuleRepository.all(p).mapError(e => RepositoryError(e.getMessage)))
