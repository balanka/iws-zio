...
object ModuleEndpoint {
  ...
  val moduleModifyAPI     = Endpoint.post(literal("module")/ string("company")).in[Module].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val moduleModifyEndpoint = moduleModifyAPI.implement(p => ZIO.logInfo(s"Modify module  ${p}") *>
    ModuleRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
