...
object CompanyEndpoint {
  ...
  val compModifyAPI     = Endpoint.post(literal("comp")/ string("company")).in[Company].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val moduleModifyEndpoint = compModifyAPI.implement(p => ZIO.logInfo(s"Modify company  ${p}") *>
    CompanyRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
