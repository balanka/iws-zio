...
object AccountEndpoint {
  ...
  val accModifyAPI     = Endpoint.post(literal("acc")/ string("company")).in[Account].out[Int].outError[RepositoryError](Status.InternalServerError)
  ...
  val accModifyEndpoint = accModifyAPI.implement(p => ZIO.logInfo(s"Modify account  ${p}") *>
    AccountRepository.modify(p._2).mapError(e => RepositoryError(e.getMessage)))
