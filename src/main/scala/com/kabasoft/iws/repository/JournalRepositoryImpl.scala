...
final class JournalRepositoryImpl(pool: ConnectionPool) extends JournalRepository with IWSTableDescriptionPostgres {
  ...
  val journals =(long("id") ++long("transid") ++ string("account") ++ string("oaccount")
    ...
    ++ bigDecimal("idebit")++ bigDecimal("debit")++ bigDecimal("icredit")++ bigDecimal("credit")++ string("currency")
    ++ string("text") ++ int("month") ++ int("year") ++ string("company")++ int("file_content")++int("modelid")
      ).table("journal2")

val (id, transid,  account, oaccount, transdate, enterdate, postingdate, period, amount, idebit, debit, icredit, credit
, currency, text, month, year, company, file, modelid  )= journals.columns
  ...
  val X =id++transid++account++oaccount++transdate++enterdate++postingdate++period++amount++idebit++debit++icredit++credit++currency++text++month++year++company++file++modelid

  def tuple2(c: Journal) =(c.id, c.transid,  c.account, c.oaccount, c.transdate, c.enterdate, c.postingdate, c.period, c.amount,
    c.idebit, c.debit, c.icredit, c.credit , c.currency,  c.text, c.month, c.year, c.company, c.file, c.modelid )
