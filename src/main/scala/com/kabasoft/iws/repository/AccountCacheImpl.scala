...
final case class AccountCacheImpl (repository: AccountRepository) extends AccountCache  {
  ...
  override def all(id: (Int, String)): ZIO[Any, RepositoryError, List[Account]] = Cache.make(
    capacity = 100000,
    timeToLive = Duration.apply(15, TimeUnit.HOURS),
    lookup = Lookup[(Int, String), Any, RepositoryError, List[Account]](repository.all)).flatMap(_.get(id))
