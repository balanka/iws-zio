...
final case class JournalRepositoryLive(postgres: Resource[Task, Session[Task]]) extends JournalRepository, MasterfileCRUD:
  ...
  override def find4Period( accountids:List[String], fromPeriod: Int, toPeriod: Int, company: String): ZIO[Any, RepositoryError, List[Journal]] =
    queryWithTx(postgres, (accountids, fromPeriod, toPeriod, company), FIND_4_ACCOUNT_PERIOD_FROM_TO_QUERY(accountids.length))
...
object JournalRepositorySQL:
  ...
  val mfDecoder: Decoder[Journal] = mfCodec2.map :
    case (id, transid, oid, account, oaccount, parentAccount, parentOAccount, transdate, enterdate, postingdate, period, amount, idebit, debit
      ...
      Journal (id, transid, oid, account, oaccount, parentAccount, parentOAccount, toInstant(transdate), toInstant(postingdate), toInstant(enterdate)
  ...
  def base =
    sql""" id, transid, oid, account, oaccount, parent_account, parent_oaccount, transdate, enterdate, postingdate, period, amount, idebit, debit
         , icredit, credit,currency, side, text, month, year, company, modelid
           FROM   journal """
    
  def ALL_BY_PERIOD: Query[Int *: String *: EmptyTuple, Journal] =
    sql"""SELECT id, transid, oid, account, oaccount,  parent_account, parent_oaccount, transdate, enterdate, postingdate, period, amount, idebit, debit
         , icredit, credit,currency, side, text, month, year, company, modelid
           FROM   journal
  ...
  def FIND_4_ACCOUNT_PERIOD_FROM_TO_QUERY (nr: Int): Query[List[String]  *: Int *: Int *: String *: EmptyTuple, Journal] =
    sql"""SELECT id, transid, oid, account, oaccount, parent_account, parent_oaccount, transdate, enterdate, postingdate, period, amount, idebit, debit
         , icredit, credit,currency, side, text, month, year, company, modelid
       FROM journal
       WHERE account IN (${varchar.list(nr)})  AND period between  $int4 and  $int4 AND  company =$varchar
       order By period desc
       """.query(mfDecoder)
       
    
  val FIND_4_PERIOD_FROM_TO_QUERY: Query[ Int *: Int *: String *: EmptyTuple, Journal] =
      sql"""SELECT id, transid, oid, account, oaccount, parent_account, parent_oaccount, transdate, enterdate, postingdate, period, amount, idebit, debit
         , icredit, credit,currency, side, text, month, year, company, modelid
       FROM journal
