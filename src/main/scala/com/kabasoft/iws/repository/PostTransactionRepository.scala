...
trait PostTransactionRepository:
   def post(models: List[Transaction], pac2Insert: List[PeriodicAccountBalance], pac2update: UIO[List[PeriodicAccountBalance]]
         , transLogEntries:List[TransactionLog], journals: List[Journal], stocks:List[Stock], newStock:List[Stock]
...
object PostTransactionRepository:
  def post(models: List[Transaction], pac2Insert:List[PeriodicAccountBalance], pac2update:UIO[List[PeriodicAccountBalance]]
           ...
           , articles: List[Article]): ZIO[PostTransactionRepository, RepositoryError, Int] =
               ZIO.serviceWithZIO[PostTransactionRepository](_.post(models, pac2Insert, pac2update, transLogEntries
                                                                          , journals, stocks, newStock, articles))
