...
final case class PartnerRepositoryLive(postgres: Resource[Task, Session[Task]]) extends PartnerRepository, MasterfileCRUD:
  ...
  override def create(c: Contact): ZIO[Any, RepositoryError, Int] = executeWithTx(postgres, c, insert, 1)
  override def create(list: List[Contact]): ZIO[Any, RepositoryError, Int] =
                         executeWithTx(postgres, list.map(Contact.encodeIt), insertAll(list.size), list.size)
  override def modify(model: Contact): ZIO[Any, RepositoryError, Int] = executeWithTx(postgres, model, Contact.encodeIt2, UPDATE, 1)
  override def modify(models: List[Contact]): ZIO[Any, RepositoryError, Int] = executeBatchWithTxK(postgres, models, UPDATE, Contact.encodeIt2)
  override def all(p: (Int, String)): ZIO[Any, RepositoryError, List[Contact]] = queryWithTx(postgres, p, ALL)
  override def getById(p: (String, Int, String)): ZIO[Any, RepositoryError, Contact] = queryWithTxUnique(postgres, p, BY_ID)
  override def getBy(ids: List[String], modelid: Int, company: String): ZIO[Any, RepositoryError, List[Contact]] =
...
private[repository] object PartnerRepositorySQL:
  ...
  val mfDecoder: Decoder[Contact] = mfCodec.map:
    case (id, name, description, street, zip, city, state, country, phone, email, company, modelid, enterdate, changedate, postingdate) =>
      Contact(id, name, description, street, zip, city, state, country, phone, email, company, modelid
        , toInstant(enterdate), toInstant(changedate), toInstant(postingdate)
       )
  

  val mfEncoder: Encoder[Contact] = mfCodec.values.contramap(Contact.encodeIt)
  ...
  def ALL_BY_ID(nr: Int): Query[(List[String], Int, String), Contact] =
    sql"""SELECT id, name, description, street, zip, city, state, country, phone, email, company, modelid, enterdate, changedate,postingdate
           FROM   partner
           WHERE id  IN ${varchar.list(nr)} AND  modelid = $int4 AND company = $varchar
           ORDER BY id ASC""".query(mfDecoder)

  val BY_ID: Query[String *: Int *: String *: EmptyTuple, Contact] =
    sql"""SELECT id, name, description,  street, zip, city, state, country, phone, email, company, modelid, enterdate, changedate,postingdate
           FROM   partner
           WHERE id = $varchar AND modelid = $int4 AND company = $varchar
           ORDER BY id ASC""".query(mfDecoder)

  val ALL: Query[Int *: String *: EmptyTuple, Contact] =
    sql"""SELECT id, name, description, street, zip, city, state, country, phone, email, company, modelid, enterdate, changedate,postingdate
           FROM   partner
           WHERE  modelid = $int4 AND company = $varchar
           ORDER BY id ASC""".query(mfDecoder)

  val insert: Command[Contact] =
  ...
  def insertAll(n:Int):Command[List[Contact.TYPE]]= sql"""INSERT INTO
  ...
  val UPDATE: Command[Contact.TYPE2] =
    sql"""UPDATE partner
