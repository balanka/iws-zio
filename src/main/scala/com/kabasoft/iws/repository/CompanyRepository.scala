...
trait CompanyRepository {
  def create(item: Company): ZIO[Any, RepositoryError, Unit]
...
object CompanyRepository {
  ...
  def create(item: Company): ZIO[CompanyRepository, RepositoryError, Unit]              =
    ZIO.service[CompanyRepository] flatMap (_.create(item))
  def create(items: List[Company]): ZIO[CompanyRepository, RepositoryError, Int]        =
