...
import com.kabasoft.iws.domain.{ModelId, PeriodicAccountBalance, ReminderBalance}
...
final case class PacRepositoryLive(postgres: Resource[Task, Session[Task]]) extends PacRepository, MasterfileCRUD:
  ...
  override def getByParent(ids: List[String], from:Int, to:Int, modelid: Int, company: String): ZIO[Any, RepositoryError, List[PeriodicAccountBalance]] =
    if (ids.isEmpty) ZIO.succeed(List.empty)
    else queryWithTx(postgres, (ids, from, to, ModelId.PERIODIC_ACCOUNT_BALANCE.modelid, company), BY_ACCOUNT_PERIOD_FROM_TO(ids.length))
  ...
  override def findBalance4paymentReminder(accountId:String, companyId: String): ZIO[Any, RepositoryError, List[ReminderBalance]] =
    queryWithTx(postgres, (accountId, companyId), BALANCE_4_PAYMENT_REMINDER)
...
object PacRepositorySQL:
  ...
  val reminderCodec = text *:  int4 *: numeric *: int4
  ...
  private val balanceDecoder: Decoder[ReminderBalance] = reminderCodec.map:
      case (id, period, balance, modelid) => ReminderBalance(id, period, balance.bigDecimal, modelid)
