...
import com.kabasoft.iws.domain.{DerivedTransaction, FinancialsTransaction, FinancialsTransactionDetails, Journal, PeriodicAccountBalance}
import com.kabasoft.iws.repository.{PacRepository, TransactionRepository, JournalRepository}

import java.time.Instant
import zio._
import zio.prelude._

final  class FinancialsServiceImpl (/*accRepo:AccountRepository,*/ pacRepo:PacRepository
            , ftrRepo:TransactionRepository, journalRepo:JournalRepository) extends FinancialsService {
...
*/
  ...
  override def postTransaction4Period(fromPeriod: Int, toPeriod: Int, company: String):ZIO[Any, RepositoryError, List[Int]] =
    (for {
  ...
  private[this] def getNewPac(pacList: List[DPAC], model: FinancialsTransaction): List[DPAC] = {
    ...
    val result:List[DPAC] = newRecords
      ...
      .toList.filterNot(x=>x.id == PeriodicAccountBalance.dummy.id)
  ...
  def creditIt(packList: List[DPAC], period: Int, line: FTDetails): Option[DPAC] =
    packList.find(pac_ => pac_.id == PeriodicAccountBalance.createId(period, line.oaccount)).map(_.crediting(line.amount))
  ...
  private[this] def getAndDebitCreditOldPacs(
    ...
    val groupedLines: List[FTDetails] = lines.groupBy(_.account).map({ case (_, v) => reduce(v,FinancialsTransactionDetails.dummy) }).toList
    val groupedOLines: List[FTDetails] = lines.groupBy(_.oaccount).map({ case (_, v) => reduce(v,FinancialsTransactionDetails.dummy)}).toList
  ...
  private[this] def createPAC(accountId: String, period: Int, currency: String, company:String): DPAC = {
    ...
    PeriodicAccountBalance.apply(PeriodicAccountBalance.createId(period, accountId),
      accountId, period, zeroAmount, zeroAmount, zeroAmount, zeroAmount, company, currency
 ...
 object FinancialsServiceImpl {
  val live: ZLayer[ PacRepository with TransactionRepository with JournalRepository
