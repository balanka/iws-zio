...
final class PostBillOfDeliveryLive(pacRepo: PacRepository
...
object PostBillOfDeliveryLive:
  val live: ZLayer[PacRepository& TransactionRepository& TransactionLogRepository& AccountRepository&
     ArticleRepository& StockRepository& PostTransactionRepository, RepositoryError, PostBillOfDelivery] =
    ZLayer.fromFunction(new PostBillOfDeliveryLive(_, _, _, _, _))
