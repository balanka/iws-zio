...
final class PostStocktakeLive(accRepo: AccountRepository
  ...
  ZIO[Any, RepositoryError, (List[Transaction], List[FinancialsTransaction], List[PeriodicAccountBalance]
    ...
    newFtr = transactions.map(buildTransaction(_, articles, accounts, suppliers, vats,
      company.purchasingClearingAcc, TransactionModelId.PAYABLES.modelid))
...
//    (transactionsx:List[Transaction], financials:List[FinancialsTransaction]) = ZIO.collectAll(newFtr).map(_.unzip) 
...
object PostStocktakeLive:
  val live: ZLayer[TransactionRepository& TransactionLogRepository& AccountRepository& VatRepository&
      SupplierRepository& ArticleRepository& StockRepository& PostTransactionRepository&FinancialsService, RepositoryError, PostStocktake] =
    ZLayer.fromFunction(new PostStocktakeLive(_, _, _, _, _, _, _))
