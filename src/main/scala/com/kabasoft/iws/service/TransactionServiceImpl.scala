...
import com.kabasoft.iws.repository.{AccountRepository, ArticleRepository, JournalRepository, PacRepository
  , PostTransactionRepository, StockRepository, TransactionLogRepository, TransactionRepository}
import zio._
...
//    pacRepo.getByIds(ids, company)
  ...
  override def postAll(ids: List[Long], company: String): ZIO[Any, RepositoryError, Int] =
    for {
      queries <- ZIO.foreach(ids)(id => ftrRepo.getByTransId((id, company))).map(_.filter(m => !m.posted))
...
//      nr <-  postTransaction(models, company, newStock, oldStocks)
 postedGoodreceiving <- postGoodreceiving.postAll(goodreceiving)
 postedBillOfDelivery <- postBillOfDelivery.postAll(bilOfDelivery)
 postedOrder <- orderService.postAll(purchaseOrder)
     // service <- ZIO.foreach( models)(TransactionService.postAll())
      //nr<- getService4Transaction(service)
...
object TransactionServiceImpl {
  val live: ZLayer[PacRepository with TransactionRepository with TransactionLogRepository with AccountRepository with PostOrder with PostGoodreceiving
    ...
    ZLayer.fromFunction(new TransactionServiceImpl(_, _, _, _))
