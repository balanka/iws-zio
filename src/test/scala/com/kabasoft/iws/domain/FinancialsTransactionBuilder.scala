package com.kabasoft.iws.domain

import com.kabasoft.iws.domain.AccountBuilder.{accountId, accountName, faccountId, faccountName, incaccountId1
  , incaccountName1, vataccountId, vataccountName}

import java.math.{BigDecimal, RoundingMode}
import java.time.Instant

object FinancialsTransactionBuilder {
  val company ="-1000"
  val transactionId = -1L// 4711L
  val transactionId2 = -2L// 4711L
  val lineTransactionId1 = 1L
  val lineTransactionId2 = 2L
  val modelid = ModelId.PAYABLES.modelid
  val modelid2 = ModelId.PAYMENT.modelid
  val period = common.getPeriod(Instant.now())
  val side = true
  val amount = new BigDecimal("100.00").setScale(2, RoundingMode.HALF_UP)
  val vat = new BigDecimal("0.19").setScale(2, RoundingMode.HALF_UP)
  val vatAmount = amount.multiply(vat)
  val text = "terms"
  val currency = "EUR"
  val costCenter = "311"
  val footText = "footText"

  val line1=  FinancialsTransactionDetails(-1, 0,  faccountId, side, incaccountId1 , amount.subtract(vatAmount), Instant.now(), text, currency, company, faccountName, incaccountName1, modelid )
  val line2=  FinancialsTransactionDetails(-1, 0,  faccountId, side, vataccountId, vatAmount, Instant.now(), text, currency, company, faccountName, vataccountName, modelid)
  val line3 = FinancialsTransactionDetails(-3, 0,  accountId, side, faccountId, amount.add(vatAmount) , Instant.now(), text, currency, company, accountName, faccountName, modelid2)
  val line4 = FinancialsTransactionDetails(1, 0,  accountId, side, faccountId, amount.add(vatAmount) , Instant.now(), text, currency, company, accountName, faccountName, modelid2)
  val line5 = FinancialsTransactionDetails(1, 1,  faccountId, side, incaccountId1, amount.subtract(vatAmount) , Instant.now(), text, currency, company, faccountName, incaccountName1, modelid2 )
  val line6 = FinancialsTransactionDetails(2, 1,  faccountId, side, vataccountId, vatAmount , Instant.now(), text, currency, company, faccountName, vataccountName, modelid2)

  val ftr1 = FinancialsTransaction(-1, "-1", "contact",  costCenter, accountId, Instant.now(), Instant.now(), Instant.now()
    , period, false, modelid, company, "comments"+modelid, footText, -1, List(line1, line2))
  val ftr2 = FinancialsTransaction( -1, "-1", "contact", costCenter, accountId, Instant.now(), Instant.now().plusMillis(2L), Instant.now()
    , period, false, modelid2, company, "comments", footText,  -1, List(line3))
  val ftr4 = FinancialsTransaction(-1, "-1", "contact", costCenter, accountId, Instant.now(), Instant.now(), Instant.now()
    , period, false, modelid2, company, "comments", footText, 0, List(line4))
  val ftr5 = FinancialsTransaction( -1, "-1", "contact", costCenter, accountId, Instant.now(), Instant.now(), Instant.now()
    , period, false, modelid2, company, "comments", footText, 0, List(line5, line6))
  //val dtransactions = ftr1.toDerive()
  val pacs = PeriodicAccountBalance.create(ftr1).distinct



}