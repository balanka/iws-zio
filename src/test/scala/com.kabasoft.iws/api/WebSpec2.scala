    ...
    def spec = suite("API test")(
      suite("AOI integration test")(
        ...
        test("Get all account all") {
          val program = runApp(appAcc.toApp @@ debug, Request.get(url = URL(!! / "acc")))
        ...
        test("Get a  bank by id") {
         ...
         test("Get all Bank statement") {
          val program = runApp(appBankStmt.toApp @@ debug, Request.get(url = URL(!! / "bs")))
          assertZIO(program)(equalTo(Vector("200 GET /bs 0ms\n")))
        },

        test("Get all cost center ") {
          val program = runApp(appCC.toApp @@ debug, Request.get(url = URL(!! / "cc")))
        ...
        test("Get a  cost center by id") {
         ...
         test("Get all users ") {
            val program = runApp(appUser.toApp @@ debug, Request.get(url = URL(!! / "user")))
        ...
        test("Get all module") {
          val program = runApp(appModule.toApp @@ debug, Request.get(url = URL(!! / "module")))
        ...
        test("Get all customer ") {
          val program = runApp(appCust.toApp @@ debug, Request.get(url = URL(!! / "cust")))
        ...
        test("Get all supplier") {
          val program = runApp(appSup.toApp @@ debug, Request.get(url = URL(!! / "sup")))
        ...
        test("Get all vat") {
          val program = runApp(appVat.toApp @@ debug, Request.get(url = URL(!! / "vat")))
