    ...
    suite("Transaction repository test with postgres test container")(
      test("insert a  transactions, modify, gets a transaction by transId and count the Nr all transactions ") {
        ...
        for {
          ...
          all <- FinancialsTransactionRepository.all(ftr1.modelid, companyId).map(_.size)
          ftr <- FinancialsTransactionRepository.getByTransId((ftr1.id, companyId))
        ...
        } yield assertTrue(oneRow == 2, all == 1, count == 3, ftrByModelIdCount == 1, nrUpdated == 3, ftr2.text == terms, nrUpdatedLines == 2)
      }
